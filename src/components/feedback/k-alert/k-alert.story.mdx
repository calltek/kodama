...
## **Interactivo**
Se puede cerrar la alerta haciendo click en el botón de cerrar y aplicando la propiedad `closable`

<Canvas>
  <Story name="Closable" argTypes={renderArgs(KAlert, {}, true)} parameters={renderParameters({showPanel: false})}>
    {() => {
      return {
        components: { KAlert },
