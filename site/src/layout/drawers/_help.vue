<template>
    <div
        id="kt_help"
        class="bg-body"
        data-kt-drawer="true"
        data-kt-drawer-name="help"
        data-kt-drawer-activate="true"
        data-kt-drawer-overlay="true"
        data-kt-drawer-width="{default:'350px', 'md': '525px'}"
        data-kt-drawer-direction="end"
        data-kt-drawer-toggle="#kd_help_toggle"
        data-kt-drawer-close="#kd_help_close"
    >
        <div class="card shadow-none rounded-0 w-100">
            <!--begin::Header-->
            <div id="kt_help_header" class="card-header">
                <h5 class="card-title fw-bold text-gray-600">
                    😗 ¿Necesitas ayuda?
                </h5>

                <div class="card-toolbar">
                    <button
                        id="kd_help_close"
                        type="button"
                        class="btn btn-sm btn-icon explore-btn-dismiss me-n5"
                    >
                        <span class="svg-icon svg-icon-2">
                            <inline-svg
                                src="/media/icons/duotune/arrows/arr061.svg"
                            />
                        </span>
                    </button>
                </div>
            </div>
            <!--end::Header-->

            <!--begin::Body-->
            <div id="kt_help_body" class="card-body">
                <!--begin::Content-->
                <div
                    id="kt_help_scroll"
                    class="hover-scroll-overlay-y d-flex flex-column"
                    data-kt-scroll="true"
                    data-kt-scroll-height="auto"
                    data-kt-scroll-wrappers="#kt_help_body"
                    data-kt-scroll-dependencies="#kt_help_header"
                    data-kt-scroll-offset="5px"
                >
                    <!--begin::Link-->
                    <div class="d-flex align-items-center mb-7">
                        <!--begin::Icon-->
                        <div
                            class="d-flex flex-center w-50px h-50px w-lg-75px h-lg-75px flex-shrink-0 rounded bg-light-warning"
                        >
                            <span
                                class="svg-icon svg-icon-warning svg-icon-2x svg-icon-lg-3x"
                            >
                                <inline-svg
                                    src="/media/icons/duotune/abstract/abs027.svg"
                                />
                            </span>
                        </div>
                        <!--end::Icon-->
                        <!--begin::Info-->
                        <div class="d-flex flex-stack flex-grow-1 ms-4 ms-lg-6">
                            <!--begin::Wrapper-->
                            <div class="d-flex flex-column me-2 me-lg-5">
                                <!--begin::Title-->
                                <a
                                    href="https://preview.keenthemes.com/metronic8/vue/docs/#/build"
                                    class="text-dark text-hover-primary fw-bolder fs-6 fs-lg-4 mb-1"
                                    >Documentación y vídeos</a
                                >
                                <!--end::Title-->
                                <!--begin::Description-->
                                <div class="text-muted fw-bold fs-7 fs-lg-6">
                                    Quiza tu duda este ya resuelta. Accede a la
                                    documentación para echar un vistazo.
                                </div>
                                <!--end::Description-->
                            </div>
                            <!--end::Wrapper-->
                            <span class="svg-icon svg-icon-gray-400 svg-icon-2">
                                <inline-svg
                                    src="/media/icons/duotune/arrows/arr064.svg"
                                />
                            </span>
                        </div>
                        <!--end::Info-->
                    </div>
                    <!--end::Link-->
                    <!--begin::Link-->
                    <div class="d-flex align-items-center mb-7">
                        <!--begin::Icon-->
                        <div
                            class="d-flex flex-center w-50px h-50px w-lg-75px h-lg-75px flex-shrink-0 rounded bg-light-primary"
                        >
                            <span
                                class="svg-icon svg-icon-primary svg-icon-2x svg-icon-lg-3x"
                            >
                                <inline-svg
                                    src="/media/icons/duotune/ecommerce/ecm007.svg"
                                />
                            </span>
                        </div>
                        <!--end::Icon-->
                        <!--begin::Info-->
                        <div class="d-flex flex-stack flex-grow-1 ms-4 ms-lg-6">
                            <!--begin::Wrapper-->
                            <div class="d-flex flex-column me-2 me-lg-5">
                                <!--begin::Title-->
                                <a
                                    href="https://preview.keenthemes.com/metronic8/vue/docs/#/utilities"
                                    class="text-dark text-hover-primary fw-bolder fs-6 fs-lg-4 mb-1"
                                    >Plugins &amp; Components</a
                                >
                                <!--end::Title-->
                                <!--begin::Description-->
                                <div class="text-muted fw-bold fs-7 fs-lg-6">
                                    Check out our 300+ in-house components and
                                    customized 3rd-party plugins.
                                </div>
                                <!--end::Description-->
                            </div>
                            <!--end::Wrapper-->
                            <span class="svg-icon svg-icon-gray-400 svg-icon-2">
                                <inline-svg
                                    src="/media/icons/duotune/arrows/arr064.svg"
                                />
                            </span>
                        </div>
                        <!--end::Info-->
                    </div>
                    <!--end::Link-->
                    <!--begin::Link-->
                    <div class="d-flex align-items-center mb-7">
                        <!--begin::Icon-->
                        <div
                            class="d-flex flex-center w-50px h-50px w-lg-75px h-lg-75px flex-shrink-0 rounded bg-light-info"
                        >
                            <span
                                class="svg-icon svg-icon-info svg-icon-2x svg-icon-lg-3x"
                            >
                                <inline-svg
                                    src="/media/icons/duotune/art/art006.svg"
                                />
                            </span>
                        </div>
                        <!--end::Icon-->
                        <!--begin::Info-->
                        <div class="d-flex flex-stack flex-grow-1 ms-4 ms-lg-6">
                            <!--begin::Wrapper-->
                            <div class="d-flex flex-column me-2 me-lg-5">
                                <!--begin::Title-->
                                <router-link
                                    to="/builder"
                                    class="text-dark text-hover-primary fw-bolder fs-6 fs-lg-4 mb-1"
                                    >Layout Builder</router-link
                                >
                                <!--end::Title-->
                                <!--begin::Description-->
                                <div class="text-muted fw-bold fs-7 fs-lg-6">
                                    Dynamically modify and preview layout
                                </div>
                                <!--end::Description-->
                            </div>
                            <!--end::Wrapper-->
                            <!--begin::Svg Icon | path: icons/duotune/arrows/arr064.svg-->
                            <span class="svg-icon svg-icon-gray-400 svg-icon-2">
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    width="24"
                                    height="24"
                                    viewBox="0 0 24 24"
                                    fill="none"
                                >
                                    <rect
                                        opacity="0.5"
                                        x="18"
                                        y="13"
                                        width="13"
                                        height="2"
                                        rx="1"
                                        transform="rotate(-180 18 13)"
                                        fill="black"
                                    ></rect>
                                    <path
                                        d="M15.4343 12.5657L11.25 16.75C10.8358 17.1642 10.8358 17.8358 11.25 18.25C11.6642 18.6642 12.3358 18.6642 12.75 18.25L18.2929 12.7071C18.6834 12.3166 18.6834 11.6834 18.2929 11.2929L12.75 5.75C12.3358 5.33579 11.6642 5.33579 11.25 5.75C10.8358 6.16421 10.8358 6.83579 11.25 7.25L15.4343 11.4343C15.7467 11.7467 15.7467 12.2533 15.4343 12.5657Z"
                                        fill="black"
                                    ></path>
                                </svg>
                            </span>
                            <!--end::Svg Icon-->
                        </div>
                        <!--end::Info-->
                    </div>
                    <!--end::Link-->

                    <router-link :to="{ name: 'changelog' }">
                        <div class="d-flex align-items-center mb-7">
                            <div
                                class="d-flex flex-center w-50px h-50px w-lg-75px h-lg-75px flex-shrink-0 rounded bg-light-danger"
                            >
                                <span
                                    class="svg-icon svg-icon-danger svg-icon-2x svg-icon-lg-3x"
                                >
                                    <inline-svg
                                        src="/media/icons/duotune/electronics/elc009.svg"
                                    />
                                </span>
                            </div>

                            <div
                                class="d-flex flex-stack flex-grow-1 ms-4 ms-lg-6"
                            >
                                <div class="d-flex flex-column me-2 me-lg-5">
                                    <a
                                        href="https://preview.keenthemes.com/metronic8/vue/docs/#/changelog"
                                        class="text-dark text-hover-primary fw-bolder fs-6 fs-lg-4 mb-1"
                                        >¿Viste las últimas novedades?</a
                                    >

                                    <div
                                        class="text-muted fw-bold fs-7 fs-lg-6"
                                    >
                                        Quizá tu duda tenga que ver con alguna
                                        funcionalidad nueva incluida
                                        recientemente.
                                    </div>
                                </div>
                                <span
                                    class="svg-icon svg-icon-gray-400 svg-icon-2"
                                >
                                    <inline-svg
                                        src="/media/icons/duotune/arrows/arr064.svg"
                                    />
                                </span>
                            </div>
                        </div>
                    </router-link>

                    <div class="mt-auto"></div>

                    <div
                        class="rounded border border-dashed border-gray-300 p-6 p-lg-8 mb-10"
                    >
                        <!--begin::Heading-->
                        <h2 class="fw-bolder mb-5">
                            Crea un ticket de soporte en
                            <a href="https://calltek.es/soporte" class=""
                                >calltek.es</a
                            >
                        </h2>
                        <!--end::Heading-->
                        <!--begin::Description-->
                        <div class="fs-5 fw-bold mb-5">
                            <span class="text-gray-500"
                                >¿No puedes resolver tu problema por tu cuenta?
                                Accede a nuestro centro de soporte y te
                                ayudaremos en todo lo que podamos.</span
                            >
                            <a
                                class="explore-link d-none"
                                href="https://keenthemes.com/licensing"
                                >FAQs</a
                            >
                        </div>
                        <!--end::Description-->
                        <!--begin::Link-->
                        <a
                            href="https://devs.keenthemes.com"
                            class="btn btn-lg explore-btn-primary w-100"
                            >Abrir incidencia</a
                        >
                        <!--end::Link-->
                    </div>
                    <!--end::Support-->
                </div>
                <!--end::Content-->
            </div>
            <!--end::Body-->
        </div>
    </div>
</template>

<script lang="ts">
    import { defineComponent } from 'vue'

    export default defineComponent({
        name: 'HelpDrawer',
        setup() {
            return {}
        }
    })
</script>

<style scoped></style>
